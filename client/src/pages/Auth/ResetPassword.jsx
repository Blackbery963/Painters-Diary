...
import {
  Mail, Lock, ArrowLeft, KeyRound, Eye, EyeOff,
  Send, Sparkles, ShieldCheck, CheckCircle, ArrowRight
} from 'lucide-react';
import { Toaster, toast } from 'sonner';

// Asset Imports
import backgroundImage from '../../assets/ResetPassword.jpg';
import logo from '../../assets/Logo.jpeg';

// ─── Step indicator ───────────────────────────────────────────────────────────
const StepIndicator = ({ current }) => {
  const steps = ['Email', 'Verify OTP', 'Old Password', 'New Password'];
  return (
...
const PasswordStrength = ({ password }) => {
  ...
  const colorClass = ['', 'bg-red-400', 'bg-yellow-400', 'bg-blue-400', 'bg-[#1f7d53]'][score];
  const textColor = ['', 'text-red-400', 'text-yellow-500', 'text-blue-500', 'text-[#1f7d53]'][score];
  ...
  return (
    <div className="mt-2 space-y-1.5">
      <div className="flex gap-1">
        {[1, 2, 3, 4].map(i => (
          <div key={i} className={`h-1 flex-1 rounded-full transition-all duration-300 ${i <= score ? colorClass : 'bg-gray-200 dark:bg-gray-700'}`} />
        ))}
      </div>
...
// ─── Password field ───────────────────────────────────────────────────────────
const PasswordField = ({ value, onChange, placeholder, show, onToggle, match }) => (
  <div className="relative group">
    ...
    <input
      ...
      className={`w-full pl-12 pr-12 py-4 bg-gray-50 dark:bg-gray-800 border-2 rounded-2xl transition-all outline-none text-gray-900 dark:text-white
        ${match === true ? 'border-[#1f7d53]' : match === false ? 'border-red-400' : 'border-transparent focus:border-[#1f7d53]'}`}
    />
    <button
      type="button"
      onClick={onToggle}
...
const ResetPassword = () => {
  ...
  const [step, setStep] = useState(1); // 1=email, 2=otp, 3=old-password, 4=new-password, 5=success
  ...
  const handleEmailSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      // TODO: await API.post('/auth/send-otp', { email });
  ...
  const handleOldPasswordSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      // TODO: await API.post('/auth/verify-password', { email, oldPassword });
      await new Promise(r => setTimeout(r, 1500));
      ...
      <div className="w-full lg:w-1/2 h-[35vh] lg:h-full relative bg-gray-900 shrink-0">
        ...
        <div className="relative z-10 p-6 lg:p-16 flex flex-col justify-center items-center lg:items-start h-full text-white">
          ...
          <h1 className="text-2xl lg:text-5xl font-bold text-center lg:text-left leading-tight">
            Account <span className="text-[#1f7d53]">Recovery</span>
          </h1>
        </div>
      </div>
            ...
            <AnimatePresence mode="wait">
                ...
                <motion.div key="s1" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.25 }}>
                  <div className="mb-8 text-center lg:text-left">
                    ...
                    <h2 className="text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white flex items-center justify-center lg:justify-start gap-2">
                      Want to Reset Your Password? <Sparkles className="w-5 h-5 text-[#1f7d53] hidden lg:block" />
                    </h2>
                    <p className="mt-2 text-gray-500 dark:text-gray-400 text-sm">
                      Enter your email and we'll send a 6-digit OTP to verify it's you.
                    </p>
                  </div>
                  ...
                  <form onSubmit={handleEmailSubmit} className="space-y-6">
                    ...
                    <Link to="/auth/login" className="flex items-center justify-center gap-2 text-sm text-gray-500 hover:text-[#1f7d53] transition-colors font-bold">
                ...
                <motion.div key="s2" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.25 }}>
                  ...
                  <form onSubmit={handleOtpSubmit} className="space-y-6">
                    ...
                    <div className="flex items-center justify-between text-sm">
                      <button type="button" onClick={() => setStep(1)} className="flex items-center gap-1.5 text-gray-500 hover:text-[#1f7d53] transition-colors font-bold">
              ...
              {step === 3 && (
                <motion.div key="s3" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.25 }}>
                  <div className="mb-8 text-center lg:text-left">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#1f7d53]/10 text-[#1f7d53] text-[10px] font-bold mb-4 uppercase tracking-widest">
                      <Lock className="w-3.5 h-3.5" /> Confirm Identity
                    </div>
                    <h2 className="text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white flex items-center justify-center lg:justify-start gap-2">
                      Current Password <Sparkles className="w-5 h-5 text-[#1f7d53] hidden lg:block" />
                    </h2>
                    <p className="mt-2 text-gray-500 dark:text-gray-400 text-sm">
                      Enter your existing password to confirm it's you before setting a new one.
                    </p>
                  </div>
                  ...
                  <form onSubmit={handleOldPasswordSubmit} className="space-y-6">
                    ...
                    <button type="button" onClick={() => setStep(2)} className="flex items-center justify-center gap-1.5 w-full text-sm text-gray-500 hover:text-[#1f7d53] transition-colors font-bold">
              ...
              {step === 4 && (
                <motion.div key="s4" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.25 }}>
                  <div className="mb-8 text-center lg:text-left">
                    ...
                    <h2 className="text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white flex items-center justify-center lg:justify-start gap-2">
                      Set New Password <Sparkles className="w-5 h-5 text-[#1f7d53] hidden lg:block" />
                    </h2>
                    <p className="mt-2 text-gray-500 dark:text-gray-400 text-sm">
                  ...
                  <form onSubmit={handleNewPasswordSubmit} className="space-y-4">
                    ...
                    <div>
                      <PasswordField
                        ...
                        match={confirmPassword ? confirmMatch : undefined}
                      />
                    ...
                    <button type="button" onClick={() => setStep(3)} className="flex items-center justify-center gap-1.5 w-full text-sm text-gray-500 hover:text-[#1f7d53] transition-colors font-bold">
                      <ArrowLeft className="w-4 h-4" /> Back
                    </button>
                  </form>
                </motion.div>
              ...
              {step === 5 && (
                ...
                >
                  ...
                  <p className="text-gray-500 dark:text-gray-400 mb-8 text-sm">
                    Your account is secure. Redirecting you to the studio...
                  </p>
