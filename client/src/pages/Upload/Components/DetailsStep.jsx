 ...
 * FILE: DetailsStep.jsx
 * PURPOSE: Step 2 of the upload flow. Handles metadata collection like 
 * title, description, smart tags, pricing, and stock.
 */
import React from 'react';
import { Tag, X, Plus, Package, Store, TagIcon } from 'lucide-react'; // Removed Trophy, added Package for stock
import { MAX_TAGS, SUGGESTED_TAGS } from './constants';
...
const DetailsStep = ({ formData, setFormData, tagInput, setTagInput, handleAddTag, handleRemoveTag, handleTagInputKeyDown }) => {
  return (
    <div className="animate-in fade-in slide-in-from-right-8 duration-500 max-w-2xl mx-auto space-y-10">
      ...
      <div>
        <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-4 flex justify-between items-center">
          <div className='flex items-center justify-center gap-2'><TagIcon size={18}/><span> Tags & Categories</span>
           </div>
          <span className="font-normal lowercase tracking-normal">{formData.tags.length}/{MAX_TAGS}</span>
        </label>
      ...
      <div className="pt-4 border-t border-zinc-100 dark:border-zinc-800">
        ...
        {/* Price and Stock Inputs (Visible only if For Sale is true) */}
        {formData.isForSale && (
          <div className="mt-6 flex flex-col sm:flex-row gap-8 animate-in slide-in-from-top-2">
            
            {/* Price Input */}
            <div className="flex-1">
              <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-2 block">Price</label>
              <div className="flex items-baseline gap-1">
                ...
                <input 
                  ...
                  placeholder="0" 
                  className="w-full bg-transparent outline-none border-none p-0 text-3xl font-light focus:ring-0 text-zinc-900 dark:text-white placeholder:text-zinc-200" 
                />
              </div>
            </div>
            ...
            {/* Stock Input */}
            <div className="flex-1">
              <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-2 block flex items-center gap-1">
                <Package size={12} /> Available Stock
              </label>
              <div className="flex items-baseline gap-1">
                <input 
                  type="number" 
