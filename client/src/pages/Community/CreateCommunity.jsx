...
import { createCommunity } from '../../service/community.service';
...
const CreateCommunityPage = () => {
  ...
  const validateStep = () => {
    if (step === 1) {
      if (!formData.logo) {
        toast.error('A community logo is required');
        return false;
      }
...
const handleSubmit = async () => {
  ...
  if (!formData.logo) {
    toast.error("Please upload a community logo before launching.");
    return;
  }
  ...
  try {
    const payload = {
      ...
      logo: formData.logo,
      coverImage: formData.cover || null
    }
                    ...
                    <div className="flex justify-between items-end">
                      <div className="flex gap-4 items-end">
                        <div className="w-24 h-24 rounded-xl bg-white dark:bg-zinc-900 p-1 shadow-md">
                          {formData.logoPreview ? (
                            <img src={formData.logoPreview} className="w-full h-full rounded-lg object-cover bg-zinc-200" alt="Avatar" />
                          ) : (
